...
MOVIE_CHOICES = (("FILM", "Film"), ("COMMERCIAL", "Commercial"))
...
class CommercialModelForm(forms.ModelForm):
    actors = forms.ModelMultipleChoiceField(
        label="Select Actors",
        widget=forms.CheckboxSelectMultiple(attrs={"class": "checkbox_1"}),
        queryset=Actor.objects.filter(is_star=False),
    )
...
class FilmModelForm(forms.ModelForm):
    actors = forms.ModelMultipleChoiceField(
        label="Select Actors",
        widget=forms.CheckboxSelectMultiple(attrs={"class": "checkbox_1"}),
        queryset=Actor.objects.filter(is_star=True),
    )
